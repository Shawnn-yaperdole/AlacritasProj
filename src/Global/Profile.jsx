...
const Profile = ({ role, setCurrentView }) => {
  ...
  const [selectedLatLon, setSelectedLatLon] = useState({ lat: 16.4023, lon: 120.5960 });
  ...
  const handleSave = async () => {
    const updatedData = { profilePic, fullName, email, phone, location, communities, defaultCommunity, bio, skills };
    ...
    <div className="profile-page flex flex-col md:flex-row justify-start w-full min-h-screen p-8 bg-gray-50 gap-12" style={{ paddingTop: "100px" }}>
        ...
        <div className="profile-pic-container relative">
          ...
          <input ref={fileInputRef} type="file" accept="image/*"
            onChange={async e => {
              ...
              try { setIsUploadingPic(true); const url = await uploadFileToCloudinary(file); setProfilePic(url); }
              catch (err) { console.error(err); setProfileError("Failed to upload."); }
              finally { setIsUploadingPic(false); e.target.value = null; }
            }} className="hidden" />
          <button className="edit-pic-btn" onClick={() => fileInputRef.current?.click()} disabled={isUploadingPic}>
            {isUploadingPic ? "Uploading..." : "Edit"}
          </button>
        </div>

        <div className="contact-info flex flex-col gap-2 text-center md:text-left">
          <div><strong>Email:</strong> {isEditingProfile ? <input value={email} onChange={e => setEmail(e.target.value)} className="rd-search-input p-1" /> : email}</div>
          <div><strong>Phone:</strong> {isEditingProfile ? <input value={phone} onChange={e => setPhone(e.target.value)} className="rd-search-input p-1" /> : phone}</div>
          <div><strong>Location:</strong> {isEditingProfile ? <input value={location} onChange={e => setLocation(e.target.value)} className="rd-search-input p-1" /> : location}</div>
        </div>
      </div>
      ...
      <div className="profile-right flex flex-col gap-6 md:w-2/3 text-center md:text-left">
        <div className="flex items-center justify-between">
          {isEditingProfile ? <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} className="text-4xl font-bold rd-search-input p-2" />
        ...
        <div className="communities flex flex-col gap-2">
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-2">
            {communities.map(c => (
              <span key={c} className={`community-badge relative ${c === selectedBarangay ? "default-community" : ""}`}>
                {c} {c === selectedBarangay && "(Default)"}
                {isEditingProfile && (
                  <button type="button" onClick={() => {
                    const updated = communities.filter(com => com !== c);
                    setCommunities(updated);
                    if (selectedBarangay === c) { setSelectedBarangay(updated[0] || ""); setDefaultCommunity(updated[0] || ""); }
                  }} className="ml-1 text-red-500 font-bold hover:text-red-700 absolute top-0 -right-2">×</button>
                )}
              </span>
            ))}
          </div>
        ...
        {showCommunityModal && (
          <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md relative">
              <button className="absolute top-2 right-2 text-gray-500 hover:text-gray-800" onClick={handleCloseCommunityModal}>✕</button>
              <h3 className="text-xl font-semibold mb-4">Join a New Community</h3>
              <input type="text" placeholder="Search Barangay..." value={barangaySearch} onChange={e => handleBarangaySearch(e.target.value)} className="border p-2 rounded w-full mb-2" />
              <div className="max-h-40 overflow-y-auto mb-4 border rounded">
                {filteredBarangays.map(b => (
                  <div key={b.name} className="p-2 hover:bg-gray-100 cursor-pointer" onClick={() => handleJoinCommunity(b)}>
                    {b.name} <button className="text-sm ml-2 px-1 py-0.5 border rounded">Join</button>
                  </div>
                ))}
              </div>
        ...
        <div className="bio flex flex-col gap-2">
          ...
          {isEditingBio ? <textarea value={bio} onChange={e => setBio(e.target.value)} className="bio-textarea p-2 border rounded resize-none w-full" rows={1} onInput={e => { e.target.style.height = "auto"; e.target.style.height = e.target.scrollHeight + "px"; }} /> : <p className="bio-text">{bio}</p>}
        </div>
